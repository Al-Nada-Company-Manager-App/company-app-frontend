...
import type { CustomerSale } from "@src/types/Dashboard/dashboard";

const CustomerSalesChart = ({data, isDark} : {data : CustomerSale[], isDark: boolean}) => {

  const formattedData = (data ?? []).map((d) => ({
    type: d.c_name ?? "Unknown",
    value: Number(d.salescount ?? 0),
  }));
