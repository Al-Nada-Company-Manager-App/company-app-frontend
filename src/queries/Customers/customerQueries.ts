...
// Update customer photo mutation
export const useUpdateCustomerPhoto = (isDark: boolean = false) => {
  const queryClient = useQueryClient();
  const { showSuccessMessage, showErrorMessage } = useThemedMessage(isDark);

  return useMutation({
    mutationFn: ({ c_id, photo }: { c_id: number; photo: File }) =>
      customerApi.updateCustomerPhoto(c_id, photo),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: customerKeys.lists() });
      showSuccessMessage("Customer photo updated successfully!", "✅");
    },
    onError: (error) => {
      console.error("Error updating customer photo:", error);
      showErrorMessage("Failed to update customer photo", "❌");
    },
  });
};
